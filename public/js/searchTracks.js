const searchTracks = async (event) => {
    ...
    try {
        console.log(`https://the-cassette-tape.herokuapp.com/api/tracks/search?q=${query}&type=${type}&limit=${limit}`);

        const response = await fetch(`https://the-cassette-tape.herokuapp.com/api/tracks/search?q=${query}&type=${type}&limit=${limit}`, {
            method: "GET"
        });
